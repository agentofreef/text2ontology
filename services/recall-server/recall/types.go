...
// JSONB. recall surfaces it in the 🎯 section so the LLM knows which params
// the strict-mode smartquery({intent, params}) call will accept for this Intent.
//
// Type semantics match agent-server/smartquery.IntentParameter (server-side
...
type MetricIntent struct {
	...
	// Strict-mode helpers (P7) — surface the Intent's defaults + param schema
	// so the LLM can call smartquery({intent, params}) without having to
	// re-read each Intent record:
