...
func FormatContext(result RecallResult, tokens []string, question string) string {
	if !result.HasMatches {
		...
		return fmt.Sprintf("## 上下文识别结果\n\n用户问题: %q\nAI 理解的关键词: [%s]\n未能从知识库匹配到以上关键词。\n\n请向用户澄清其问题意图，并提示可用的查询方向。不要猜测或编造数据对象和口径。",
			question, tokStr)
	}
	...
	if len(hints) > 0 || len(gbHints) > 0 {
		...
		sb.WriteString("- 例：用户问「NA地区在所有GEO中的占比」→ `filters:[{prop:GEO,op:=,value:\"N.A.\"}]` **且** `groupBy:[\"GEO\", ...]`，全局分母由口径的 `pivot_percent_scope=global` 处理\n")
...
func formatMetricIntent(sb *strings.Builder, mi MetricIntent, _ []string) {
	...
	sb.WriteString(fmt.Sprintf("#### `口径:%s` %s\n", mi.Name, title))
	if mi.Description != "" {
