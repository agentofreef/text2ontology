...
你的任务不是回答数据查询，而是帮助用户**构造**项目的本体资产 (OD / 指标 / Link)。
每次会话围绕一个核心 OD 展开。OD 启用后，你会主动追问是否要附加指标或 OD 间 Link，形成级联流。
...
**指标管理（3 个）：**
- propose_intent — 提议新指标草稿（pending，用户点"启用"才落库）
- update_intent  — 修改指标字段 / 关键词
- delete_intent  — 删除指标
...
3. OD 启用成功后 → 问："要不要为 <OdName> 设个指标？(例：sum(订单量) 按月分组)"
   - 用户回 "好" → 进入 propose_intent 流
   - 用户回 "skip" / "算了" → 跳过
4. 指标启用（或跳过）→ 调用 inspect(mode=fk_candidates) 找 FK 候选 → 问："<OdName> 与 <CandidateOd> 的 <col> 看起来能 JOIN，要建 link 吗?"
...
func builderV2Tools() []llmclient.ToolDef {
	return []llmclient.ToolDef{
		...
		{Name: "propose_intent", Description: "在已激活 OD 上提议一个指标（查询模板，pending 状态，mark=false）。triggerKeywords 仅存在工具结果 JSON 中，启用时才会写 lakehouse_keyword。返回 pending_confirmation: true。", Parameters: M{
			"type":     "object",
			"required": []string{"objectId", "name", "canonicalMetric", "autoGroupBy", "triggerKeywords"},
			"properties": M{
				"objectId":        M{"type": "string", "description": "指标绑定的已激活 OD 的 UUID"},
				"name":            M{"type": "string", "description": "指标名称，如 Order.Quantity"},
				...
				}, "description": "指标永远追加的过滤条件"},
				"autoGroupBy":           M{"type": "array", "items": M{"type": "string"}, "description": "指标必须保留的 groupBy 维度（后端强制注入）"},
				...
				"triggerKeywords":       M{"type": "array", "items": M{"type": "string"}, "minItems": 1, "description": "命中此指标的触发词列表（启用时才会写 lakehouse_keyword）"},
			},
		}},
		{Name: "update_intent", Description: "修改已有指标的字段、新增或删除触发关键词（pending 或 active 均可，partial update）。", Parameters: M{
			"type":     "object",
			"required": []string{"intentId"},
			"properties": M{
				"intentId": M{"type": "string", "description": "要修改的指标 UUID"},
				"edits": M{
					"type":        "object",
					"description": "指标顶级字段的修改（partial update）：可含 name / canonicalMetric / canonicalFilters / autoGroupBy / pivotOn 等",
				},
				"keywordAdds": M{
					"type":        "array",
		...
		{Name: "delete_intent", Description: "删除一个指标（同时删除其触发关键词 lakehouse_keyword 记录）。", Parameters: M{
			"type":     "object",
			"required": []string{"intentId"},
			"properties": M{
				"intentId": M{"type": "string", "description": "要删除的指标 UUID"},
			},
		}},
