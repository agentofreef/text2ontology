...
你的任务不是回答数据查询，而是帮助用户**构造**项目的本体资产 (OD / Intent / Link)。
每次会话围绕一个核心 OD 展开。OD 启用后，你会主动追问是否要附加 Metric Intent 或 OD 间 Link，形成级联流。
...
- 利用返回的启发式指标（is_likely_id / is_likely_machine_code / is_likely_timestamp / value_distribution）形成基于真实数据的具体问题，而非抽象问"业务口径"。
...
**Intent 管理（3 个）：**
- propose_intent — 提议新 Intent 草稿（pending，用户点"启用"才落库）
- update_intent  — 修改 Intent 字段 / 关键词
- delete_intent  — 删除 Intent
...
3. OD 启用成功后 → 问："要不要为 <OdName> 设个 Metric Intent？(例：sum(订单量) 按月分组)"
   - 用户回 "好" → 进入 propose_intent 流
   - 用户回 "skip" / "算了" → 跳过
4. Intent 启用（或跳过）→ 调用 inspect(mode=fk_candidates) 找 FK 候选 → 问："<OdName> 与 <CandidateOd> 的 <col> 看起来能 JOIN，要建 link 吗?"
...
func builderV2Tools() []llmclient.ToolDef {
	return []llmclient.ToolDef{
		...
		{Name: "propose_intent", Description: "在已激活 OD 上提议一个 Metric Intent（查询模板，pending 状态，mark=false）。triggerKeywords 仅存在工具结果 JSON 中，启用时才会写 lakehouse_keyword。返回 pending_confirmation: true。", Parameters: M{
			"type":     "object",
			"required": []string{"objectId", "name", "canonicalMetric", "autoGroupBy", "triggerKeywords"},
			"properties": M{
				"objectId":        M{"type": "string", "description": "Intent 绑定的已激活 OD 的 UUID"},
				"name":            M{"type": "string", "description": "Intent 名称，如 Order.Quantity"},
				...
				}, "description": "Intent 永远追加的过滤条件"},
				"autoGroupBy":           M{"type": "array", "items": M{"type": "string"}, "description": "Intent 必须保留的 groupBy 维度（后端强制注入）"},
				...
				"triggerKeywords":       M{"type": "array", "items": M{"type": "string"}, "minItems": 1, "description": "命中此 Intent 的触发词列表（启用时才会写 lakehouse_keyword）"},
			},
		}},
		{Name: "update_intent", Description: "修改已有 Metric Intent 的字段、新增或删除触发关键词（pending 或 active 均可，partial update）。", Parameters: M{
			"type":     "object",
			"required": []string{"intentId"},
			"properties": M{
				"intentId": M{"type": "string", "description": "要修改的 Intent UUID"},
				"edits": M{
					"type":        "object",
					"description": "Intent 顶级字段的修改（partial update）：可含 name / canonicalMetric / canonicalFilters / autoGroupBy / pivotOn 等",
				},
				"keywordAdds": M{
					"type":        "array",
		...
		{Name: "delete_intent", Description: "删除一个 Metric Intent（同时删除其触发关键词 lakehouse_keyword 记录）。", Parameters: M{
			"type":     "object",
			"required": []string{"intentId"},
			"properties": M{
				"intentId": M{"type": "string", "description": "要删除的 Intent UUID"},
			},
		}},
