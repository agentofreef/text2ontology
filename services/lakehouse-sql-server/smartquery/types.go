...
type QuerySpec struct {
	...
	// IntentHint carries Metric-Intent metadata selected upstream by the
	// agent (priority + keyword gate). The lakehouse pipeline applies it
	// via PassApplyIntentHint to enforce canonical_metric / auto_group_by
	// without hitting lakehouse_metric_intent itself. nil = no intent fired.
