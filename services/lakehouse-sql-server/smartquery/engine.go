...
func NormalizeQuerySpec(raw map[string]interface{}) QuerySpec {
	...
	// IntentHint — passed through verbatim from agent-server. The agent
	// resolved the intent already; lakehouse just applies the rules via
	// PassApplyIntentHint without re-querying the metric_intent table.
	if ih, ok := raw["intentHint"].(map[string]interface{}); ok {
		hint := &IntentHint{}
		if v, ok := ih["intentId"].(string); ok {
			hint.IntentID = v
		}
		if v, ok := ih["name"].(string); ok {
			hint.Name = v
		}
		if v, ok := ih["canonicalMetric"].(string); ok {
			hint.CanonicalMetric = v
		}
		if agb, ok := ih["autoGroupBy"].([]interface{}); ok {
			for _, g := range agb {
				hint.AutoGroupBy = append(hint.AutoGroupBy, fmt.Sprintf("%v", g))
			}
		}
		if v, ok := ih["replaceGroupBy"].(bool); ok {
			hint.ReplaceGroupBy = v
		}
		if v, ok := ih["addShareColumnSafe"].(bool); ok {
			hint.AddShareColumnSafe = v
		}
		if v, ok := ih["defaultOrderByLabel"].(string); ok {
			hint.DefaultOrderByLabel = v
		}
		if v, ok := ih["defaultOrderByDir"].(string); ok {
			hint.DefaultOrderByDir = v
		}
		if v, ok := ih["defaultLimit"].(float64); ok && v > 0 {
			hint.DefaultLimit = int(v)
		}
		if cf, ok := ih["canonicalFilters"].([]interface{}); ok {
			for _, item := range cf {
				if fm, ok := item.(map[string]interface{}); ok {
					...
					if prop != "" {
						hint.CanonicalFilters = append(hint.CanonicalFilters, FilterItem{Prop: prop, Op: op, Value: val})
					}
				}
			}
		}
		// Only attach when at least one field is set; an empty hint object
		// would otherwise trigger PassApplyIntentHint with nothing to do
		// (harmless but noisy in logs).
		if hint.IntentID != "" || hint.CanonicalMetric != "" || len(hint.AutoGroupBy) > 0 ||
			hint.DefaultOrderByLabel != "" || hint.DefaultLimit > 0 || len(hint.CanonicalFilters) > 0 {
