...
- **lakehouse-sql-server** `:18094` — SmartQuery 引擎(deterministic 本体 → SQL 编译器)
...
# 1. 拷贝示例 env 文件
#    (默认值可以直接跑通本地试用;
#     正式部署前请改 POSTGRES_PASSWORD / INTERNAL_TOKEN / GRAFANA_ADMIN_PASSWORD)
cp .env.example .env
cp .env.shared.example .env.shared

# 2. 构建并启动整套(schema 自动 apply,首次 1-3 分钟)
docker compose --env-file .env.shared up -d

# 3. 等 ~30 秒服务启动,然后探活
for p in 18080 18090 18092 18093 18094 18095 18096; do
  printf "  $p: "; curl -fsS localhost:$p/healthz || echo "(启动中)"
done
```
