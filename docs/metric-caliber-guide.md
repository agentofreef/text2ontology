...
## 口径不是孤立的:它的另一半叫「任务可达器」

策展口径只解决了"系统**能**说什么"。还有一半:用户提问时,**在 LLM 发挥之前**,系统先做一次确定性判定——已授权口径的集合,**能不能逐项覆盖这个问题**?

- 能 → 进入 smartquery,正常回答。
- 不能 → 立刻停下,直接告诉用户"缺哪个维度、最相近的口径是哪几条、补法是什么"。**绝不让 LLM 用沾边的口径硬答**。

这就是**任务可达器**(也叫 任务触达器)。它把"模型答错了"这种含糊失败,翻译成"本体里少了什么"这种**有地址的可修复 bug**。

> 口径 = 白名单,任务可达器 = 门卫。两个一起才是封闭逻辑。

## 延伸阅读

- **[`docs/reachability-judge-guide.md`](reachability-judge-guide.md)** — 任务可达器:**为什么有它**(理念)+ **它怎么跑**(技术)。
